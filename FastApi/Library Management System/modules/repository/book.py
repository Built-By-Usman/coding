...
def get(id:int,db:Session):
    book=db.query(BookModel).where(BookModel.id==id).first()
    if not book:
...
def create(request:BookSchema,db:Session):
    ...
    if user:
        book=BookModel(name=request.name,author=request.author,yearOfPublication=request.yearOfPublication,user_id=request.user_id)
